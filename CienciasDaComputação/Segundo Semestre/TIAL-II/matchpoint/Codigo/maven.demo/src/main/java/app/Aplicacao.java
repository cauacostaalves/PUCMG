...
public class Aplicacao {
    ...
    public static void main(String[] args) {
        ...
        // CADASTRO DO GRUPO
        post("/cadastrarGrupo", (request, response) -> {
            try {
            
                // Parseia o corpo da requisição JSON
                String body = request.body();
                Gson gson = new Gson();
                Grupo novoGrupo = gson.fromJson(body, Grupo.class);
        
                // Verifica duplicidade
                boolean duplicado = grupoService.verGrupoDuplicado(novoGrupo.getNome());
                if (duplicado) {
                ...
                if (sucesso) {
                    ...
                    return "{\"erro\":\"Erro ao cadastrar grupo.\"}";
                }
            } catch (Exception e) {
                e.printStackTrace();
                response.status(500); // Internal Server Error
                return "{\"erro\":\"Erro ao processar o cadastro do grupo.\"}";
            }
        });
        ...
        // Rota para retornar todos os grupos, exceto aqueles criados pelo usuário
        // logado
