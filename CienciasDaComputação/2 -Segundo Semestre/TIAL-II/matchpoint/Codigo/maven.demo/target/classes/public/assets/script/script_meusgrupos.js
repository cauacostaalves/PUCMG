document.addEventListener("DOMContentLoaded", function () {
    ...
    function createGroupCard(grupo, isCriador) {
        ...
        let esporteNome;
        if (grupo.id_esporte === 1) {
            esporteNome = 'Futebol';
        } else if (grupo.id_esporte === 2) {
            esporteNome = 'Basquete';
        } else if (grupo.id_esporte === 3) {
            esporteNome = 'Vôlei';
        } else {
            esporteNome = 'Esporte Desconhecido';  // Caso o id_esporte não corresponda a nenhum valor
        }

        const sport = document.createElement('p');        
        sport.textContent = `Esporte: ${esporteNome}`;
        ...
        date.textContent = `Descrição: ${grupo.descricao}`;
        ...
        if (isCriador) {
            ...
            leaveButton.textContent = 'Sair do Evento';
    ...
    function showConfirmDelete(card, groupId, deleteButton) {
        ...
        confirmText.textContent = 'Tem certeza que deseja deletar este evento?';
